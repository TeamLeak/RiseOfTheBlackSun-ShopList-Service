...
// Config описывает конфигурацию сервера, базы данных и админки.
type Config struct {
	Server struct {
...
func runMigrate(configPath string) {
	...
	initDB(conf)
	if err := db.AutoMigrate(&ShopPackage{}); err != nil {
...
func runServe(configPath string) {
	...
	// Публичный эндпоинт для получения товаров (с in-memory кэшем)
	router.GET("/products", func(c *gin.Context) {
		cacheMutex.RLock()
		if productCache != nil {
			c.JSON(http.StatusOK, productCache)
			cacheMutex.RUnlock()
			return
		}
		cacheMutex.RUnlock()
